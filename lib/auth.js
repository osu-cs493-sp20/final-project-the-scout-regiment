...
exports.validateSubmission = async function (userId, assignmentId, res) {
  try {
    const user = await getUserById(userId);
    ...
    if (user && (user.role === 'admin' || user.role === 'instructor')) {
      ...
      const course = await getCourseById(assignment.courseId, false, false);
      if (course && userId !== course.instructorId && user.role !== 'admin') {
        res.status(403).send({
          error: "Unauthorized to access the specified resource"
        });
        return 0;
